...
- **Authentication/Authorization**: Session-based authentication via Passport.js with Replit OIDC, HTTP-only cookies, and a three-tier role system (Admin, Editor, Viewer). Includes a two-phase auth initialization for robust deployment health checks and lazy route loading to prevent module-load crashes when production secrets are missing.
  - **Instant Health Check Response (November 2025)**: Root endpoint (`/`) uses Accept header differentiation to provide instant JSON responses for Replit autoscale health probes (no Accept header or non-HTML) while serving the SPA HTML to browsers (Accept: text/html). Combined with lazy route loading and a SPA catch-all route (`app.get('*', ...)`), this ensures health checks pass in <5ms even during background initialization, preventing deployment timeouts. Tested response time: ~4.7ms for health checks.
