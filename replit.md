...
- **Full-Pass Export System** (October 2025): Comprehensive export functionality generating ZIP bundles containing strategic analysis and EPM program data in multiple formats (Markdown, PDF, DOCX, JSON, CSV). Backend service (`server/services/export-service.ts`) loads data from strategic understanding, journey sessions, strategy versions, EPM programs, and task assignments; generates professional reports with marked library for MD→HTML conversion, puppeteer for PDF generation (--no-sandbox, A4), docx package for Word documents, and proper CSV escaping. ZIP structure includes report.md, report.pdf, report.docx, data/strategy.json, data/epm.json, data/assignments.csv, data/workstreams.csv. API endpoint GET /api/exports/full-pass with authentication, ownership verification via strategyVersions.userId and epmPrograms.userId, streaming responses. Frontend `ExportFullReportButton` component with loading states, blob download, toast notifications integrated into Analysis Repository and EPM Programs list pages.
