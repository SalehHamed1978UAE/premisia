...
- **Journey Launcher Modal**: Intelligent modal system for initiating additional strategic analysis on existing strategies. Features two modes: Full Journey (6 pre-planned journeys) and Single Framework (surgical analysis). Includes journey-aware readiness checks with journey-specific minimums (BMI/BMC: 0/0, others: 3 references/5 entities). For follow-on journeys, automatically builds rich strategic summaries aggregating prior decisions, insights, references, and analysis from all completed sessions instead of re-using raw baseline input. Conditional UI shows "Run Now" for interactive execution or "Start in Background" when sufficient context exists. Integrates with Universal Background Jobs for non-blocking execution.
