...
- **Five Whys Carousel Interface**: Interactive root cause analysis using Five Whys methodology with anti-bias mechanisms
  - **Business-Focused Analysis**: Explicit prompt engineering forbids cultural/anthropological reasoning, requires business strategy focus
    - Forbidden topics: Cultural dynamics, hierarchical respect, face-saving, power distance, organizational culture theory
    - Required topics: Market saturation, competitive positioning, product-market fit, pricing pressure, CAC, go-to-market strategy
    - Validates each option ensures it explains business competitive dynamics, not cultural accommodation
  - **Evidence-Based Decision Support**: Each option includes structured evidence to evaluate causal accuracy
    - Supporting Evidence (2-3 points): Data, metrics, or reasoning validating the causal explanation
    - Counter-Arguments (2-3 points): Evidence challenging the explanation or suggesting alternative paths
    - Consideration (1 sentence): Neutral insight comparing trade-offs to help user evaluate if path is worth exploring
    - Collapsible UI: Consideration always visible (blue), supporting evidence (green), counter-arguments (amber)
  - **Root Cause Validation**: Keyword-based validation blocks cultural observations from proceeding to research
    - Detects cultural keywords (cultural, culture, tradition, hierarchical respect, face-saving, power distance, etc.)
    - Shows warning modal with "Go Back" button if cultural content detected
    - Ensures only business problems (competitive, market, operational issues) advance to research phase
  ...
  - Timeout handling: Shows "Generated Level 1 completely, Level 2 generating..." if >30 seconds
...
- **Anti-Confirmation Bias Research**: Web search actively challenges assumptions to avoid confirming pre-existing beliefs
  - **Query Generation Strategy**: For any claim/assumption in input, generates BOTH validating AND challenging queries
    - Baseline queries (2-3): General market/industry context
    - Validating queries (2): Evidence supporting input claims
    - Challenging queries (2-3): Evidence contradicting or questioning input assumptions
  - **Keyword Detection**: Automatically detects language/cultural differentiation claims and adds targeted challenging queries
    - Language detection: /arabic|language|multilingual|localization/i triggers "English vs Arabic [market] business language statistics"
    - Cultural detection: /cultural|culture|islamic|traditional/i triggers "[market] enterprise software buyer preferences data"
  - **Synthesis Prioritization**: Contradictory findings weighted HIGHER than confirming evidence in final analysis
    - Explicitly highlights when research contradicts input assumptions
    - Example: If input claims Arabic but research shows "English dominates 95% of UAE business," that finding appears FIRST with HIGH confidence
