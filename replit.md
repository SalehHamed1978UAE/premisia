...
- **Strategic Consultant & EPM Integration**:
    ...
    - **Statement Repository with Delete & Archive** (Oct 2025): Analysis repository browser displays all strategic statements with AI-generated titles. Each card includes archive and delete action buttons. Delete functionality features reusable AlertDialog confirmation with cascading deletes (statement + all analyses + entities). Available at three levels: (1) individual analysis deletion from detail view, (2) analysis deletion from full report pages, (3) statement deletion (removes all analyses) from repository browser. Backend endpoints ensure referential integrity with proper 404 handling and success notifications.
