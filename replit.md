...
- **EPM V2 Engine (Jan 2026)**: Backend-only migration that rewires the Strategic Consultant's `/convert-to-epm` endpoint to use Journey Builder's EPMSynthesizer instead of the legacy EPMConverter. Benefits: industry-appropriate content (no template contamination), proper FTE decimals, validated dependencies, context-aware risks/benefits. The EPMAdapter (`server/strategic-consultant-v2/epm-adapter.ts`) bridges legacy format to V2 engine. Controlled via `USE_EPM_V2_ENGINE` env var with automatic fallback to legacy if V2 fails.
