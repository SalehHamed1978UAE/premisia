...
## 10. Timeline & Gantt Chart Generation (CPM Scheduler)

The timeline is built using **Critical Path Method (CPM)** - same as the original system.

```typescript
// server/services/multi-agent/scheduling/cpm-scheduler.ts

export interface WorkstreamScheduleInput {
  id: string;
  name: string;
  estimatedDurationMonths: number;
  dependencies: string[];  // IDs of workstreams this depends on
...
export interface Timeline {
  ...
  totalMonths: number;
  criticalPath: string[];  // Workstream IDs on critical path
  ganttData: GanttRow[];   // For UI rendering
}
...
export class CPMScheduler {
   ...
   * 3. Identify critical path (where float = 0)
   */
  schedule(
    ...
    for (const ws of workstreams) {
      earliestFinish.set(ws.id, earliestStart.get(ws.id)! + ws.estimatedDurationMonths);
    }
    ...
    for (const ws of workstreams) {
      latestStart.set(ws.id, latestFinish.get(ws.id)! - ws.estimatedDurationMonths);
    }

    // Calculate float and identify critical path
    const criticalPath: string[] = [];
    const scheduled: ScheduledWorkstream[] = workstreams.map(ws => {
      const esMonth = earliestStart.get(ws.id)!;
      const efMonth = earliestFinish.get(ws.id)!;
      const lsMonth = latestStart.get(ws.id)!;
      const lfMonth = latestFinish.get(ws.id)!;
    ...
    criticalPath.sort((a, b) =>
      earliestStart.get(a)! - earliestStart.get(b)!
    );

    // Generate phases (group workstreams into logical phases)
    const phases = this.generatePhases(scheduled, projectEndMonth);
  ...
  private forwardPass(
    ...
    for (const ws of sorted) {
      if (ws.dependencies.length === 0) {
        // No dependencies - can start at month 0
        ...
        const maxDependencyEnd = Math.max(
          ...ws.dependencies.map(depId => {
            ...
            return earliestStart.get(depId)! + dep.estimatedDurationMonths;
          })
        );
  ...
  private backwardPass(
    ...
    for (const ws of sorted) {
      ...
      if (myDependents.length === 0) {
        // Nothing depends on me - can finish at project end
        ...
        const minDependentStart = Math.min(
          ...myDependents.map(depId => {
            const dep = workstreams.find(w => w.id === depId)!;
            return latestFinish.get(depId)! - dep.estimatedDurationMonths;
          })
        );
  ...
  private generatePhases(scheduled: ScheduledWorkstream[], totalMonths: number): Phase[] {
    // Divide into 3 phases: Planning, Execution, Validation
    ...
    return [
      {
        ...
        endDate: this.addMonths(scheduled[0]?.startDate, phaseLength),
      ...
      },
      {
        ...
        startDate: this.addMonths(scheduled[0]?.startDate, phaseLength),
        endDate: this.addMonths(scheduled[0]?.startDate, phaseLength * 2),
      ...
      },
      {
        ...
        startDate: this.addMonths(scheduled[0]?.startDate, phaseLength * 2),
        endDate: this.addMonths(scheduled[0]?.startDate, totalMonths),
  ...
  private generateGanttData(scheduled: ScheduledWorkstream[], totalMonths: number): GanttRow[] {
    return scheduled.map(ws => ({
      ...
      // For Gantt rendering: percentage positions
...
### Integration with Round 4 (Resource & Timeline)

Round 4's output feeds into the CPM scheduler:

```typescript
// In orchestrator.ts, after Round 4 synthesis

private async processRound4Timeline(
  roundOutputs: RoundOutputs,
  businessContext: BusinessContext
): Promise<Timeline> {

  // Extract workstream definitions from Rounds 1-3
  const workstreamDefs = this.extractWorkstreamDefinitions(roundOutputs);

  // Round 4 agents provide duration estimates and dependencies
  const round4Output = roundOutputs[4];

  // Merge into schedule inputs
  const scheduleInputs: WorkstreamScheduleInput[] = workstreamDefs.map(ws => ({
    id: ws.id,
    name: ws.name,
    estimatedDurationMonths: round4Output.durations[ws.id] || 2,
    dependencies: round4Output.dependencies[ws.id] || ws.dependencies || [],
    deliverables: ws.deliverables.map((d, i) => ({
      id: d.id,
      name: d.name,
      relativeMonth: Math.floor((i + 1) * (round4Output.durations[ws.id] || 2) / ws.deliverables.length)
    }))
  }));

  // Run CPM scheduling
  const scheduler = new CPMScheduler();
  const { scheduled, timeline } = scheduler.schedule(
    scheduleInputs,
    new Date()  // Program starts today
  );

  // Store scheduled workstreams for EPM assembly
  this.scheduledWorkstreams = scheduled;
...
The `ganttData` array provides everything the UI needs:
...
interface GanttRow {
  ...
  dependencies: string[];
  isOnCriticalPath: boolean;
  startPercent: number;      // For positioning bar
...
- Rows for each workstream
- Horizontal bars positioned by `startPercent`, sized by `widthPercent`
- Critical path items highlighted (red/bold)
...
├── scheduling/
│   ├── cpm-scheduler.ts       # Critical Path Method scheduler
│   ├── gantt-generator.ts     # Gantt chart data generation
│   └── dependency-graph.ts    # Dependency graph utilities
├── assembly/
