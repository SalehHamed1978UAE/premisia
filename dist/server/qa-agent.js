...
export class QAAgent {
    ...
    async reviewCode(request) {
        ...
        const requirementsVerification = await this.verifyRequirements(request.builderResponse, request.originalRequirements);
        ...
        const gaps = await this.findGaps(request.builderResponse, request.originalRequirements);
        ...
        const securityIssues = await this.findSecurityIssues(request.builderResponse, request.context?.securityLevel || 'standard');
        ...
        // Collect all issues
        const allIssues = [...gaps, ...bugs, ...securityIssues, ...qualityIssues];
        // Identify critical blockers
        const criticalBlockers = allIssues
            .filter(i => i.severity === 'critical')
            .map(i => i.description);
        // Count unsatisfied requirements
        const unsatisfiedRequirements = requirementsVerification.filter(r => !r.satisfied);
        ...
        const overallConfidence = this.calculateVerdictConfidence(verdict, requirementsVerification, allIssues);
        // Generate recommendations
        const recommendations = this.generateRecommendations(allIssues, unsatisfiedRequirements);
        // Generate summary
        const reviewSummary = this.generateSummary(verdict, requirementsVerification, allIssues, criticalBlockers);
     ...
     */
    async verifyRequirements(response, requirements) {
        const verification = [];
        for (const requirement of requirements) {
            // Check if BuilderResponse claims it's fulfilled
            const builderClaim = response.requirementsFulfilled.find(r => r.requirement === requirement);
            // ADVERSARIAL: Don't trust builder's self-assessment
            // Verify independently
            const actuallyMet = this.independentlyVerifyRequirement(requirement, response, builderClaim);
            const gaps = [];
            if (!actuallyMet) {
     ...
     */
    independentlyVerifyRequirement(requirement, response, builderClaim) {
        ...
        const hasEvidence = requirementKeywords.some(keyword => codeContent.includes(keyword.toLowerCase()));
    ...
    extractKeywords(requirement) {
        ...
        return words.filter(w => w.length > 3 && !['the', 'and', 'with', 'from', 'that', 'this'].includes(w));
    }
    /**
     * Find gaps in implementation
     */
    async findGaps(response, requirements) {
        const gaps = [];
        ...
        for (const artifact of response.code) {
            if (artifact.content.includes('placeholder') ||
     ...
     */
    async findSecurityIssues(response, securityLevel) {
        const issues = [];
        for (const artifact of response.code) {
     ...
     */
    calculateVerdictConfidence(verdict, verification, issues) {
        if (verdict === 'FAIL') {
     ...
     * Generate recommendations
     */
    generateRecommendations(issues, unsatisfiedRequirements) {
     ...
     */
    generateSummary(verdict, verification, issues, criticalBlockers) {
