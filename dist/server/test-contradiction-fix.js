    ...
    try {
        ...
        for (const contradiction of mockContradictions) {
            ...
            if (sourceEntity) {
                ...
                // Create relationship
                await strategicUnderstandingService.createRelationship(sourceEntity.id, contradictionEntity.id, 'contradicts', 'high', contradiction.contradictedBy.join('; '), 'bmc_agent');
        ...
        const relationships = allRelationships.filter(rel => entityIds.includes(rel.fromEntityId) || entityIds.includes(rel.toEntityId));
...
//# sourceMappingURL=test-contradiction-fix.js.map