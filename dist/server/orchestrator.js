    ...
    async processTask(task) {
        ...
        try {
            ...
            while (iteration <= task.maxRetries) {
                ...
                builderResponse = await this.runBuilder(provider, task, ontologyContext, qaReview // Pass previous QA feedback for retry
                );
                ...
                if (currentSession) {
                    for (let i = 0; i < builderResponse.requirements.length; i++) {
                        ...
                        if (req.satisfied) {
                            const criterionIndex = currentSession.successCriteria.findIndex(c => c.includes(`Builder satisfies: ${req.requirement}`));
                            if (criterionIndex !== -1) {
                ...
                // QA: Review code
                qaReview = await this.runQA(provider, task.requirements, builderResponse);
                ...
                if (sessionAfterQA) {
                    for (let i = 0; i < qaReview.requirementsVerification.length; i++) {
                        ...
                        if (verification.satisfied) {
                            const criterionIndex = sessionAfterQA.successCriteria.findIndex(c => c.includes(`QA verifies: ${verification.requirement}`));
                            if (criterionIndex !== -1) {
                                await this.executiveAgent.completeCriterion(criterionIndex);
                            }
                        }
                ...
                if (qaReview.verdict === "PASS") {
                    ...
                    if (passSession) {
                        const passCriterionIndex = passSession.successCriteria.findIndex(c => c.includes("QA verdict is PASS"));
