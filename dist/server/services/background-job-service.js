    ...
    async getRunningJobs(userId) {
        const result = await dbConnectionManager.withFreshConnection(async (db) => {
            return await db
                .select()
                .from(backgroundJobs)
                .where(and(eq(backgroundJobs.userId, userId), inArray(backgroundJobs.status, ['pending', 'running'])))
                .orderBy(desc(backgroundJobs.createdAt));
        });
        return result;
    }
    ...
    async getJobsByRelatedEntity(relatedEntityId, relatedEntityType) {
        const result = await dbConnectionManager.withFreshConnection(async (db) => {
            return await db
                .select()
                .from(backgroundJobs)
                .where(and(eq(backgroundJobs.relatedEntityId, relatedEntityId), eq(backgroundJobs.relatedEntityType, relatedEntityType)))
                .orderBy(desc(backgroundJobs.createdAt));
        });
        return result;
    }
    ...
    async cleanupOldJobs(daysOld = 7) {
        ...
        const result = await dbConnectionManager.withFreshConnection(async (db) => {
            ...
            const jobsToDelete = await db
                ...
                .from(backgroundJobs)
                .where(and(inArray(backgroundJobs.status, ['completed', 'failed']), 
