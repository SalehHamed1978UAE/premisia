...
export class ReferenceService {
    ...
    normalizeReference(raw, userId, usage, options) {
        ...
        return {
            ...
            topics: raw.topics || [],
            confidence: (raw.confidence !== null && raw.confidence !== undefined) ? String(Math.min(Math.max(raw.confidence, 0), 1)) : null,
        ...
        for (const ref of refs) {
            ...
            if (seen.has(key)) {
                ...
                seen.set(key, {
                    ...
                    confidence: mergedConfidence !== null && mergedConfidence !== undefined ? String(mergedConfidence) : null,
                });
            ...
            try {
                ...
                if (existing) {
                    ...
                    const [updatedRef] = await db.update(references)
                        ...
                        extractedQuotes: mergedQuotes,
                        topics: mergedTopics,
                        confidence: mergedConfidence !== null && mergedConfidence !== undefined ? mergedConfidence.toString() : null,
                        lastValidated: new Date(),
                        updatedAt: new Date(),
                    })
    ...
    async updateMetadataCache(understandingId) {
        ...
        const refCount = await db.select({ count: sql `count(*)` })
            .from(references)
