...
import { journeySessions, epmPrograms, taskAssignments, frameworkInsights, } from '@shared/schema';
 ...
 */
export async function generateFullPassExport(request, outputStream) {
...
async function loadExportData(sessionId, versionNumber, programId, userId) {
    ...
    if (journeySession) {
        const [fiveWhysInsight] = await db.select()
            ...
            .where(and(eq(frameworkInsights.sessionId, journeySession.id), eq(frameworkInsights.frameworkName, 'five_whys')))
    ...
    return {
        ...
        strategy: {
            ...
            journeySession,
            strategyVersion,
            decisions: strategyVersion?.decisions ?? [],
            fiveWhysTree,
            whysPath,
            clarifications,
        },
            ...
            if (bmc.revenueStreams) {
                ...
                if (typeof bmc.revenueStreams === 'string') {
                    ...
                    bmc.revenueStreams.streams.forEach((rev) => {
                        ...
                        if (rev.pricingModel)
                            lines.push(`  - Pricing: ${rev.pricingModel}`);
                    });
                    lines.push('');
                }
            }
        ...
        if (insights.portersForces) {
            ...
            if (forces.competitiveRivalry || forces.competitive_rivalry) {
                ...
                if (typeof rivalry === 'string') {
                    ...
                    if (rivalry.intensity)
                        lines.push(`**Intensity:** ${rivalry.intensity}\n`);
                    if (rivalry.factors && Array.isArray(rivalry.factors)) {
                        rivalry.factors.forEach((f) => lines.push(`- ${f}`));
                        lines.push('');
                    }
                }
            }
            if (forces.threatOfNewEntrants || forces.threat_of_new_entrants) {
                ...
                if (typeof threat === 'string') {
                    ...
                    if (threat.barriers && Array.isArray(threat.barriers)) {
                        ...
                        threat.barriers.forEach((b) => lines.push(`- ${b}`));
                        lines.push('');
                    }
                }
            }
            if (forces.bargainingPowerOfSuppliers || forces.supplier_power) {
                ...
                if (typeof power === 'string') {
                    ...
                    if (power.power)
                        lines.push(`**Power Level:** ${power.power}\n`);
                    if (power.factors && Array.isArray(power.factors)) {
                        power.factors.forEach((f) => lines.push(`- ${f}`));
                        lines.push('');
                    }
                }
            }
            ...
            if (forces.threatOfSubstitutes || forces.threat_of_substitutes) {
                ...
                if (typeof threat === 'string') {
                    ...
                    if (threat.substitutes && Array.isArray(threat.substitutes)) {
                        threat.substitutes.forEach((s) => lines.push(`- ${s}`));
                        lines.push('');
                    }
                }
            }
    ...
    if (pkg.epm?.program) {
        ...
        if (benefits) {
            ...
            if (benefits.benefits && benefits.benefits.length > 0) {
                lines.push('\n### Expected Benefits\n');
                benefits.benefits.forEach((b, idx) => {
        ...
        if (qaPlan) {
            ...
            if (qaPlan.standards && qaPlan.standards.length > 0) {
                lines.push('\n**Quality Standards:**\n');
                qaPlan.standards.forEach((std) => lines.push(`- ${std}`));
                lines.push('');
            }
...
async function generateDocxReport(pkg) {
    ...
    if (pkg.epm?.program) {
        ...
        if (execSummary) {
            ...
            if (execSummary.overview || execSummary.summary) {
                sections.push(new Paragraph({ text: execSummary.overview || execSummary.summary }));
            }
            if (execSummary.objectives && execSummary.objectives.length > 0) {
                sections.push(new Paragraph({
        ...
        if (resourcePlan) {
            ...
            if (resourcePlan.internalTeam && resourcePlan.internalTeam.length > 0) {
                ...
                resourcePlan.internalTeam.forEach((r) => {
                    internalTeamRows.push(new TableRow({
                        children: [
            ...
            if (resourcePlan.externalResources && resourcePlan.externalResources.length > 0) {
                ...
                resourcePlan.externalResources.forEach((r) => {
                    externalResourcesRows.push(new TableRow({
                        children: [
        ...
        if (financialPlan) {
            ...
            if (financialPlan.costBreakdown && financialPlan.costBreakdown.length > 0) {
                ...
                financialPlan.costBreakdown.forEach((item) => {
                    ...
                    const amount = typeof item.amount === 'number' ? `$${item.amount.toLocaleString()}` : item.amount;
                    const pct = item.percentage || '-';
                    costBreakdownRows.push(new TableRow({
                        children: [
        ...
        if (stakeholders) {
            ...
            if (Array.isArray(stakeholderArray) && stakeholderArray.length > 0) {
                ...
                stakeholderArray.forEach((s) => {
                    ...
                    const interest = s.interest || s.interestLevel || '-';
                    const strategy = s.engagement || s.strategy || '-';
                    stakeholderRows.push(new TableRow({
                        children: [
                            new TableCell({ children: [new Paragraph({ text: name })] }),
        ...
        if (exitStrategy) {
            ...
            if (exitStrategy.criteria && exitStrategy.criteria.length > 0) {
                ...
                exitStrategy.criteria.forEach((c) => {
                    sections.push(new Paragraph({
                        text: `• ${c}`,
                        bullet: { level: 0 },
                    }));
    ...
    // Footer
    sections.push(new Paragraph({ text: '' }), new Paragraph({
        children: [
            new TextRun({
 ...
 */
function generateAssignmentsCsv(assignments) {
 ...
 */
function generateWorkstreamsCsv(workstreams) {
 ...
 */
function generateResourcesCsv(resourcePlan) {
  ...
  `);
    ...
    `);
      ...
      `);
        ...
        if (insights.bmcBlocks) {
            const bmc = insights.bmcBlocks;
            const renderBmcBlock = (title, data) => {
                if (!data)
                    return '';
                if (typeof data === 'string') {
                    ...
                    return `<div class="mb-3"><h4>${title}</h4><ul>${data.map((item) => `<li>${escapeHtml(item)}</li>`).join('')}</ul></div>`;
                }
                else if (data.segments) {
                    return `<div class="mb-3"><h4>${title}</h4><ul>${data.segments.map((seg) => `<li><strong>${escapeHtml(seg.name || 'Segment')}:</strong> ${escapeHtml(seg.description || '')}</li>`).join('')}</ul></div>`;
                }
                else if (data.propositions) {
                    return `<div class="mb-3"><h4>${title}</h4><ul>${data.propositions.map((vp) => `<li><strong>${escapeHtml(vp.title || 'Value Proposition')}:</strong> ${escapeHtml(vp.description || '')}</li>`).join('')}</ul></div>`;
                }
                else if (data.channels) {
                    return `<div class="mb-3"><h4>${title}</h4><ul>${data.channels.map((ch) => `<li><strong>${escapeHtml(ch.name || 'Channel')}:</strong> ${escapeHtml(ch.description || '')}</li>`).join('')}</ul></div>`;
                }
                else if (data.relationships) {
                    return `<div class="mb-3"><h4>${title}</h4><ul>${data.relationships.map((rel) => `<li><strong>${escapeHtml(rel.type || 'Relationship')}:</strong> ${escapeHtml(rel.description || '')}</li>`).join('')}</ul></div>`;
                }
                else if (data.streams) {
                    return `<div class="mb-3"><h4>${title}</h4><ul>${data.streams.map((rev) => `<li><strong>${escapeHtml(rev.name || 'Revenue Stream')}:</strong> ${escapeHtml(rev.description || '')}${rev.pricingModel ? `<br><em>Pricing: ${escapeHtml(rev.pricingModel)}</em>` : ''}</li>`).join('')}</ul></div>`;
                }
                else if (data.resources) {
                    return `<div class="mb-3"><h4>${title}</h4><ul>${data.resources.map((res) => `<li><strong>${escapeHtml(res.name || 'Resource')}:</strong> ${escapeHtml(res.description || '')}</li>`).join('')}</ul></div>`;
                }
                else if (data.activities) {
                    return `<div class="mb-3"><h4>${title}</h4><ul>${data.activities.map((act) => `<li><strong>${escapeHtml(act.name || 'Activity')}:</strong> ${escapeHtml(act.description || '')}</li>`).join('')}</ul></div>`;
                }
                else if (data.partnerships) {
                    return `<div class="mb-3"><h4>${title}</h4><ul>${data.partnerships.map((part) => `<li><strong>${escapeHtml(part.partner || 'Partner')}:</strong> ${escapeHtml(part.description || '')}</li>`).join('')}</ul></div>`;
                }
                else if (data.costs) {
                    return `<div class="mb-3"><h4>${title}</h4><ul>${data.costs.map((cost) => `<li><strong>${escapeHtml(cost.category || 'Cost')}:</strong> ${escapeHtml(cost.description || '')}</li>`).join('')}</ul></div>`;
                }
                return '';
            };
            contentParts.push(`
        <div class="card">
          ...
          <div class="card-content">
            ...
            <div class="mb-3">
              ...
              <ul>
                ${insights.bmcContradictions.map((cont) => {
                if (typeof cont === 'string') {
            ...
            <div>
              <h4>Critical Gaps</h4>
              <ul>
                ${insights.businessModelGaps.map((gap) => {
                if (typeof gap === 'string') {
        ...
        if (insights.portersForces) {
            ...
            const renderForce = (title, force) => {
                ...
                if (typeof force === 'string') {
                    ...
                    if (force.intensity)
                        html += `<p><strong>Intensity:</strong> ${escapeHtml(force.intensity)}</p>`;
                    if (force.level)
                        html += `<p><strong>Level:</strong> ${escapeHtml(force.level)}</p>`;
                    if (force.power)
                        html += `<p><strong>Power:</strong> ${escapeHtml(force.power)}</p>`;
                    if (force.factors && Array.isArray(force.factors)) {
                        html += `<ul>${force.factors.map((f) => `<li>${escapeHtml(f)}</li>`).join('')}</ul>`;
                    }
                    if (force.barriers && Array.isArray(force.barriers)) {
                        html += `<p><strong>Entry Barriers:</strong></p><ul>${force.barriers.map((b) => `<li>${escapeHtml(b)}</li>`).join('')}</ul>`;
                    }
                    if (force.substitutes && Array.isArray(force.substitutes)) {
                        html += `<ul>${force.substitutes.map((s) => `<li>${escapeHtml(s)}</li>`).join('')}</ul>`;
                    }
                    html += '</div>';
                    return html;
                }
      ...
      `);
        ...
        if (insights.trendFactors || insights.externalForces) {
            const factors = insights.trendFactors ?? insights.externalForces ?? {};
            const renderPestleCategory = (category, data) => {
                if (!data)
                    return '';
                ...
                if (typeof data === 'string') {
                    ...
                    html += `<ul>${data.map((item) => `<li>${escapeHtml(item)}</li>`).join('')}</ul>`;
                }
                else {
                    if (data.trends && Array.isArray(data.trends)) {
                        html += `<p><strong>Trends:</strong></p><ul>${data.trends.map((t) => `<li>${escapeHtml(t)}</li>`).join('')}</ul>`;
                    }
                    if (data.opportunities && Array.isArray(data.opportunities)) {
                        html += `<p><strong>Opportunities:</strong></p><ul>${data.opportunities.map((o) => `<li>${escapeHtml(o)}</li>`).join('')}</ul>`;
                    }
                    if (data.risks && Array.isArray(data.risks)) {
                        html += `<p><strong>Risks:</strong></p><ul>${data.risks.map((r) => `<li>${escapeHtml(r)}</li>`).join('')}</ul>`;
                    }
                }
                html += '</div>';
                return html;
        ...
        <div class="card">
          ...
          <div class="card-content">
            ...
            <div>
              ...
              <ul>
                ${execSummary.successCriteria.map((criteria) => `<li>${escapeHtml(criteria)}</li>`).join('')}
              </ul>
            </div>
            ` : ''}
          </div>
          ...
          <div class="card-content">
            ...
            <div>
              ...
              <table>
                ...
                <tbody>
                  ${resourcePlan.externalResources.map((r) => `
                    <tr>
          ...
          <div class="card-content">
            ...
            <div class="mb-4">
              ...
              <table>
                ...
                <tbody>
                  ${financialPlan.costBreakdown.map((item) => {
                const category = item.category || item.name || 'Other';
                const amount = typeof item.amount === 'number' ? `$${item.amount.toLocaleString()}` : item.amount;
                const pct = item.percentage || '-';
                return `
              ...
              ${gates.map((gate, idx) => `
                <div class="mb-4">
                  ...
                  <div class="mt-2">
                    ...
                    <ul>
                      ${gate.criteria.map((c) => `<li>${escapeHtml(c)}</li>`).join('')}
                    </ul>
                  </div>
                  ` : ''}
                  ...
                  <div class="mt-2">
                    ...
                    <ul>
                      ${gate.deliverables.map((d) => `<li>${escapeHtml(d)}</li>`).join('')}
                    </ul>
                  </div>
                  ` : ''}
                </div>
        ...
        <div class="card">
          ...
          <div class="card-content">
            ...
            <div class="mb-4">
              ...
              <ul>
                ${qaPlan.standards.map((std) => `<li>${escapeHtml(std)}</li>`).join('')}
              </ul>
            </div>
            ` : ''}
        ...
        <div class="card">
          ...
          <div class="card-content">
            ...
            <div class="mb-4">
              ...
              <ul>
                ${exitStrategy.criteria.map((c) => `<li>${escapeHtml(c)}</li>`).join('')}
              </ul>
            </div>
            ` : ''}
