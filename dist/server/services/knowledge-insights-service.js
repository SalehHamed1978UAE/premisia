 ...
 */
import { db } from '../db';
import { strategyVersions, strategicEntities, references, journeySessions, } from '@shared/schema';
 ...
 */
export async function getSimilarStrategiesFromPostgres(understandingId, userId) {
    try {
        ...
        try {
          ...
          `,
                ...
                .where(and(eq(strategyVersions.userId, userId), currentSessionIds.length > 0
        ...
        const results = await Promise.all(similarStrategies.map(async (strategy) => {
 ...
 */
export async function getIncentivesFromPostgres(understandingId, userId) {
    try {
        ...
        const incentiveEntities = await db
            ...
            .where(and(eq(strategicEntities.understandingId, understandingId), sql `${strategicEntities.type} IN ('opportunity', 'constraint')`))
            .limit(10);
        ...
        const results = await Promise.all(incentiveEntities.map(async (entity) => {
            // Decrypt encrypted fields
 ...
 * - Dedupes by URL
 */
export async function getEvidenceFromPostgres(understandingId, userId) {
    try {
        ...
        const sessionReferences = await db
            ...
            .from(references)
            .where(and(sql `${references.sessionId} IN (${sql.join(sessionIds.map(id => sql `${id}`), sql `, `)})`, eq(references.userId, userId)))
        ...
        // Transform to response format with decryption
        const results = await Promise.all(Array.from(urlMap.values()).map(async (ref) => {
            // Decrypt encrypted fields
 ...
 */
export async function getInsightsForSession(understandingId, userId) {
...
//# sourceMappingURL=knowledge-insights-service.js.map