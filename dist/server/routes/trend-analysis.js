 ...
 */
router.post('/:understandingId', async (req, res) => {
    ...
    try {
        ...
        if (result.references && result.references.length > 0) {
            try {
                ...
                const normalized = result.references.map((ref, idx) => referenceService.normalizeReference(ref, userId, { component: 'pestle_trends', claim: ref.description }, { understandingId, sessionId }));
 ...
 */
router.get('/:understandingId/status', async (req, res) => {
    const { understandingId } = req.params;
 ...
 */
router.get('/:understandingId/latest', async (req, res) => {
    const { understandingId } = req.params;
    try {
        const [insight] = await db
            .select()
            .from(frameworkInsights)
            .where(and(eq(frameworkInsights.understandingId, understandingId), eq(frameworkInsights.frameworkName, 'PESTLE')))
