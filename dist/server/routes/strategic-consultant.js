...
router.post('/check-ambiguities', async (req, res) => {
    try {
        ...
        const result = await ambiguityDetector.detectAmbiguities(fullInputForAmbiguity, locationResult.questions);
        res.json(result);
    }
    catch (error) {
...
router.post('/journeys/execute', async (req, res) => {
    try {
        ...
        const { journeySessionId, versionNumber } = await journeyOrchestrator.startJourney(understanding.id, journeyType, userId);
...
router.post('/decisions/select', async (req, res) => {
    try {
        ...
        const updated = await versionManager.updateVersion(sessionId, versionNumber, selectedDecisions);
...
router.post('/convert-to-epm', async (req, res) => {
    try {
        ...
        const program = await epmConverter.convertToEPM(version.analysisData, version.decisionsData, version.selectedDecisions);
...
router.post('/versions/compare', async (req, res) => {
    try {
        ...
        const comparison = await versionManager.compareVersions(sessionId, versionA, versionB);
        res.json({
            success: true,
            comparison,
        });
...
router.post('/integrate/:sessionId/:versionNumber', async (req, res) => {
    try {
        ...
        try {
            ...
            const result = await epmIntegrator.integrateToEPMSuite(version.programStructure, userId, sessionId, version.id);
...
router.post('/whys-tree/expand', async (req, res) => {
    try {
        ...
        if (isCustom && customOption) {
            ...
            expandedBranches = await whysTreeGenerator.generateCustomBranches(customOption, selectedPath, input, sessionId, currentDepth);
        }
        else {
            // For AI-generated options, use normal expand logic
            expandedBranches = await whysTreeGenerator.expandBranch(nodeId, selectedPath, input, sessionId, currentDepth, parentQuestion);
        }
        res.json({
            expandedBranches,
        });
...
router.post('/whys-tree/finalize', async (req, res) => {
    try {
        ...
        const insights = await whysTreeGenerator.analyzePathInsights(input, selectedPath.map((option, index) => ({
            id: `node-${index}`,
            question: '',
            option,
            depth: index + 1,
...
router.get('/research/stream/:sessionId', async (req, res) => {
    ...
    const versionNumber = req.query.versionNumber ? parseInt(req.query.versionNumber) : undefined;
    ...
    try {
        ...
        const findings = await marketResearcher.synthesizeFindingsPublic(rootCause, input, whysPath, searchResults, topSources, sourceContents);
        ...
        try {
            ...
            if (understanding && req.user?.id) {
                ...
                const normalizedReferences = findingsWithValidation.sources.map((source) => referenceService.normalizeReference(source, userId, { component: 'research.pestle', claim: source.description || source.title }, { understandingId: understanding.id, sessionId }));
...
router.post('/analyze-enhanced', async (req, res) => {
    try {
        ...
        const enhancedAnalysis = await strategyAnalyzer.analyzeWithResearch(sessionId, rootCause, whysPath, existingAnalysisData.research, input);
        await storage.updateStrategyVersion(version.id, {
            analysisData: {
                ...existingAnalysisData,
...
router.post('/decisions/generate-with-research', async (req, res) => {
    try {
        ...
        const researchInformedDecisions = await decisionGenerator.generateDecisionsWithResearch(originalAnalysis, originalInput, existingAnalysisData.research, existingAnalysisData.enhanced_analysis.portersAnalysis);
        ...
        if (result.references && result.references.length > 0 && sessionId) {
            try {
                ...
                if (understanding) {
                    ...
                    const normalized = result.references.map((ref, idx) => referenceService.normalizeReference(ref, userId, { component: 'bmc_research', claim: ref.description }, { understandingId: understanding.id, sessionId }));
    ...
    try {
        ...
        try {
            if (result.references && result.references.length > 0 && understanding) {
                ...
                const normalized = result.references.map((reference) => referenceService.normalizeReference(reference, userId, {
        ...
        let version = null; // Declare version at outer scope
        try {
            // Get userId from journey session instead of falling back to "system"
            const journeySession = await getJourneySessionByUnderstandingSessionId(sessionId);
            const userId = journeySession?.userId || req.user?.claims?.sub;
            ...
            const versionNumberFromQuery = req.query.versionNumber ? parseInt(req.query.versionNumber) : undefined;
...
router.post('/five-whys/coach', async (req, res) => {
    try {
        const { sessionId, rootQuestion, previousWhys, candidate, userQuestion, conversationHistory } = req.body;
...
router.post('/journeys/execute-background', async (req, res) => {
    try {
        ...
        const { journeySessionId, versionNumber } = await journeyOrchestrator.startJourney(targetUnderstandingId, journeyType, userId);
...
router.post('/journeys/run-now', async (req, res) => {
    try {
        ...
        const { journeySessionId, versionNumber } = await journeyOrchestrator.startJourney(understandingId, journeyType, userId);
...
//# sourceMappingURL=strategic-consultant.js.map