...
function boostConfidenceWithDecisions(baseConfidence, userDecisions) {
    ...
    if (userDecisions.revenueModel)
        completedFields++;
    decisionFields++;
    if (userDecisions.channelPriorities?.length > 0)
        completedFields++;
    decisionFields++;
    if (userDecisions.partnershipStrategy)
        completedFields++;
    decisionFields++;
    ...
    if (userDecisions.riskTolerance)
        completedFields++;
    decisionFields++;
    if (userDecisions.investmentCapacityMin !== undefined)
        completedFields++;
    decisionFields++;
    ...
    if (userDecisions.validatedAssumptions?.length > 0)
        completedFields++;
    decisionFields++;
    if (userDecisions.concerns?.length > 0)
        completedFields++;
    decisionFields++;
...
router.get('/epm/progress/:progressId', (req, res) => {
...
router.post('/decisions', async (req, res) => {
    try {
        const { strategyVersionId, primaryCustomerSegment, revenueModel, channelPriorities, partnershipStrategy, riskTolerance, investmentCapacityMin, investmentCapacityMax, timelinePreference, successMetricsPriority, validatedAssumptions, concerns, topPriorities, goDecision, decisionRationale, } = req.body;
...
router.post('/epm/generate', async (req, res) => {
...
async function processEPMGeneration(strategyVersionId, decisionId, prioritizedOrder, progressId, req) {
    ...
    try {
        ...
        if (decisionId) {
            ...
            if (decision) {
                ...
                if (decision.revenueModel)
                    page1Fields++;
                if (Array.isArray(decision.channelPriorities) && decision.channelPriorities.length > 0)
                    page1Fields++;
                ...
                if (!decision.riskTolerance)
                    missingFields.push('Risk Tolerance');
                if (!decision.timelinePreference)
                    missingFields.push('Timeline Preference');
            ...
            try {
                ...
                if (understanding) {
                    const [journeySession] = await db
                        ...
                        .where(and(eq(journeySessions.understandingId, understanding.id), eq(journeySessions.versionNumber, version.versionNumber)))
                        .limit(1);
                    if (journeySession && journeySession.journeyType === 'business_model_innovation') {
                        ...
                        if (journeyDef?.summaryBuilder) {
                            ...
                            const summary = journeySummaryService.buildSummary(journeyDef.summaryBuilder, context, {
                            ...
                            if (autoCaptureEnabled) {
                                ...
                                setImmediate(async () => {
                                    try {
                                        const { fetchJourneySessionData, sanitizeGoldenRecordData, saveGoldenRecordToFile, } = await import('../utils/golden-records-service');
                                        ...
                                        const existingRecords = await db
                                            .select()
                                            .from(goldenRecords)
                                            .where(eq(goldenRecords.journeyType, 'business_model_innovation'))
        ...
        if (jobId) {
            await backgroundJobService.failJob(jobId, error).catch(err => console.error('[EPM Generation] Job failure update failed:', err));
        }
...
router.get('/epm/:id', async (req, res) => {
    try {
        const { id } = req.params;
...
router.get('/epm/:id/session', async (req, res) => {
    try {
        const { id } = req.params;
        // Get the program
        const [program] = await db
...
router.patch('/epm/:id', async (req, res) => {
    try {
        const { id } = req.params;
...
router.post('/epm/:id/finalize', async (req, res) => {
    try {
        const { id } = req.params;
        const [program] = await db
            .select()
...
router.post('/epm/batch-delete', async (req, res) => {
    try {
        const { ids } = req.body;
...
router.post('/epm/batch-archive', async (req, res) => {
    try {
...
router.post('/epm/batch-export', async (req, res) => {
    try {
        const { ids } = req.body;
        if (!ids || !Array.isArray(ids) || ids.length === 0) {
            return res.status(400).json({ error: 'Invalid request: ids array is required' });
...
//# sourceMappingURL=strategy-workspace.js.map