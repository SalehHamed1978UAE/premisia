...
import { researchCaptureWrapper } from '../services/research-capture-wrapper.js';
...
export class MarketResearcher {
    ...
    convertToReferences(sources, findings, category) {
        ...
        for (const source of sources) {
            ...
            relatedFindings.forEach(f => {
                ...
                if (keywords.includes('market'))
                    topics.add('market analysis');
                if (keywords.includes('competitive'))
                    topics.add('competitive landscape');
                if (keywords.includes('regulatory'))
                    topics.add('regulatory');
                if (keywords.includes('customer') || keywords.includes('buyer'))
                    topics.add('customer behavior');
            });
            references.push({
                title: source.title,
                url: source.url,
    ...
    async conductResearch(sessionId, rootCause, input, whysPath, captureContext) {
        ...
        const findings = await this.synthesizeFindings(rootCause, input, whysPath, searchResults, topSources, sourceContents);
        const allFindings = [
...
- {"query": "successful English-only enterprise software UAE", "purpose": "test if Arabic is necessary", "type": "challenging"}`;
    ...
    async performWebSearch(queries, captureContext) {
        const searchPromises = queries.map(async (queryObj) => {
            ...
            if (captureContext) {
                try {
                    const { result: data } = await researchCaptureWrapper.captureWebSearch(queryObj.query, captureContext, searchFn);
                    const results = (data.organic || []).map((result) => ({
                        url: result.link,
                        title: result.title,
                        snippet: result.snippet || '',
    ...
    async fetchSourceContent(sources, captureContext) {
        ...
        const fetchPromises = sources.map(async (source) => {
            ...
            try {
                ...
                // If capture context is provided, wrap with raw capture
                if (captureContext) {
                    const captured = await researchCaptureWrapper.captureContentFetch(source.url, captureContext, fetchFn);
...
//# sourceMappingURL=market-researcher.js.map