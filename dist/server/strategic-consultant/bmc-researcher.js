import { BMCQueryGenerator } from './bmc-query-generator';
import { MarketResearcher } from './market-researcher';
import { AssumptionExtractor } from './assumption-extractor';
import { AssumptionValidator } from './assumption-validator';
...
export class BMCResearcher {
    ...
    async conductBMCResearch(input, sessionId, sink) {
        ...
        const assumptionResults = searchResults.filter(r => assumptionOnlyQueries.some(q => q.query === r.query));
        ...
        const sourceContents = await this.marketResearcher.fetchSourceContentPublic(topSources.slice(0, 3));
        ...
        const assumptionFindings = assumptionResults.flatMap(sr => (sr.results || []).map((r) => `${r.title}: ${r.snippet}`));
        const contradictionResult = await this.assumptionValidator.detectContradictions(assumptions, assumptionFindings // Context-preserved findings
        );
        ...
        const [customerBlock, valueBlock, revenueBlock, channelsBlock, relationshipsBlock, resourcesBlock, activitiesBlock, partnersBlock, costBlock] = await Promise.all([
            this.synthesizeBlock('customer_segments', 'Customer Segments', querySet.customer_segments, [
                ...searchResults.filter(r => querySet.customer_segments.some(q => q.query === r.query)),
                ...assumptionResults // Include assumption findings in all blocks
            ], sourceContents, input, contradictionResult.contradictions),
            this.synthesizeBlock('value_propositions', 'Value Propositions', querySet.value_propositions, [
                ...searchResults.filter(r => querySet.value_propositions.some(q => q.query === r.query)),
                ...assumptionResults
            ], sourceContents, input, contradictionResult.contradictions),
            this.synthesizeBlock('revenue_streams', 'Revenue Streams', querySet.revenue_streams, [
                ...searchResults.filter(r => querySet.revenue_streams.some(q => q.query === r.query)),
                ...assumptionResults
            ], sourceContents, input, contradictionResult.contradictions),
            this.synthesizeBlock('channels', 'Channels', querySet.channels, [
                ...searchResults.filter(r => querySet.channels.some(q => q.query === r.query)),
                ...assumptionResults
            ], sourceContents, input, contradictionResult.contradictions),
            this.synthesizeBlock('customer_relationships', 'Customer Relationships', querySet.customer_relationships, [
                ...searchResults.filter(r => querySet.customer_relationships.some(q => q.query === r.query)),
                ...assumptionResults
            ], sourceContents, input, contradictionResult.contradictions),
            this.synthesizeBlock('key_resources', 'Key Resources', querySet.key_resources, [
                ...searchResults.filter(r => querySet.key_resources.some(q => q.query === r.query)),
                ...assumptionResults
            ], sourceContents, input, contradictionResult.contradictions),
            this.synthesizeBlock('key_activities', 'Key Activities', querySet.key_activities, [
                ...searchResults.filter(r => querySet.key_activities.some(q => q.query === r.query)),
                ...assumptionResults
            ], sourceContents, input, contradictionResult.contradictions),
            this.synthesizeBlock('key_partnerships', 'Key Partnerships', querySet.key_partnerships, [
                ...searchResults.filter(r => querySet.key_partnerships.some(q => q.query === r.query)),
                ...assumptionResults
            ], sourceContents, input, contradictionResult.contradictions),
            this.synthesizeBlock('cost_structure', 'Cost Structure', querySet.cost_structure, [
                ...searchResults.filter(r => querySet.cost_structure.some(q => q.query === r.query)),
                ...assumptionResults
            ], sourceContents, input, contradictionResult.contradictions),
        ]);
        const blocks = [
    ...
    async validateAllContradictions(contradictions, userInputEntities) {
        ...
        const results = await Promise.all(contradictions.map(async (contradiction) => {
            ...
            const validationResults = await Promise.all(contradiction.contradictedBy.map(evidence => this.validateContradiction(sourceEntity.claim, evidence)));
...
Step 1: Same concept? Step 2: Different values?`;
    ...
    async storeBMCFindingsInGraph(understandingId, sourceEntities, blocks, contradictions, criticalGaps) {
        ...
        try {
            ...
            const findingClaims = blocks.flatMap(block => block.findings.slice(0, 3).map(finding => finding.fact));
            ...
            await dbConnectionManager.retryWithBackoff(async (db) => {
                ...
                for (const block of blocks) {
                    for (const finding of block.findings.slice(0, 3)) {
                        await strategicUnderstandingService.createEntityWithEmbedding(db, understandingId, {
                        ...
                        }, embeddings[embeddingIndex++], 'bmc_agent');
                    }
                }
                ...
                for (const result of validatedContradictions) {
                    ...
                    if (validation.isContradiction) {
                        ...
                        const contradictionEntity = await strategicUnderstandingService.createEntityWithEmbedding(db, understandingId, {
                            type: 'research_finding',
                        ...
                        // Create relationship
                        await strategicUnderstandingService.createRelationshipDirect(db, sourceEntity.id, contradictionEntity.id, 'contradicts', contradiction.validationStrength === 'STRONG' ? 'high' : contradiction.validationStrength === 'MODERATE' ? 'medium' : 'low', contradiction.recommendation, 'bmc_agent', {
                            semanticValidation: {
    ...
    async performParallelWebSearch(queries, sink) {
        ...
        return throttler.throttleAll(searchTasks, (taskIndex) => ({ query: queries[taskIndex].query, results: [] }));
    }
    ...
    async synthesizeBlock(blockType, blockName, queries, searchResults, sourceContents, originalInput, contradictions = []) {
        const searchSummary = searchResults.map(sr => `Query: ${sr.query}\nResults: ${sr.results?.map((r) => `- ${r.title}: ${r.snippet}`).join('\n') || 'No results'}`).join('\n\n');
...
${contradictions.map(c => `❌ "${c.assumption}" - Research shows: ${c.contradictedBy.slice(0, 2).join('; ')}`).join('\n')}
...
- "weak" if limited or uncertain evidence`;
    ...
    async synthesizeOverallBMC(blocks, originalInput, contradictions = []) {
        const blockSummary = blocks.map(b => `${b.blockName} (${b.confidence} confidence):\n${b.description}\nGaps: ${b.gaps.join(', ') || 'None identified'}`).join('\n\n');
        const contradictionSummary = contradictions.length > 0
            ? `\n\n🚨 CONTRADICTED ASSUMPTIONS:\n${contradictions.map(c => `- "${c.assumption}" (${c.impact} impact${c.investmentAmount ? `, Investment: ${c.investmentAmount}` : ''})\n  Research found: ${c.contradictedBy.join('; ')}\n  Recommendation: ${c.recommendation}`).join('\n')}`
            : '';
...
//# sourceMappingURL=bmc-researcher.js.map