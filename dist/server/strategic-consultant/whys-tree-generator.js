...
export class WhysTreeGenerator {
    ...
    async generateTree(input, sessionId) {
        ...
        const level1Branches = await this.generateLevelInParallel(rootQuestion, { input, history: [] }, 1);
        const level2BranchesPromises = level1Branches.map(level1Node => this.generateLevelInParallel(level1Node.question, { input, history: [{ question: rootQuestion, answer: level1Node.option }] }, 2, level1Node.id).then(level2Branches => {
  ...
  "question": "Why [phrase this naturally]?"
    ...
    validateAnswerQuality(option, question) {
        ...
        if (optionWords.length > 5) {
            const consecutiveMatches = questionWords.filter(word => word.length > 3 && normalizedOption.includes(word)).length;
  ...
  "branches": [
    {
      ...
      "consideration": "Neutral one-sentence insight to help them compare"
            ...
            if (validBranches.length >= 3) {
                ...
                const nodes = validBranches.map((branch) => ({
                    id: randomUUID(),
    ...
    async expandBranch(nodeId, selectedPath, input, sessionId, currentDepth, parentQuestion) {
        ...
        const history = Array.isArray(selectedPath) && selectedPath.length > 0 && typeof selectedPath[0] === 'string'
            ? selectedPath.map((answer, idx) => ({
                question: `Step ${idx + 1}`, // Placeholder for old format
                answer
            }))
            : selectedPath;
        const nextDepth = currentDepth + 1;
        const branches = await this.generateLevelInParallel(parentQuestion, { input, history }, nextDepth, nodeId);
        return branches;
    }
    async generateCustomBranches(customOption, selectedPath, input, sessionId, currentDepth) {
        ...
        const branches = await this.generateLevelInParallel(nextQuestion, { input, history }, nextDepth);
        return branches;
    }
    async generateFullPath(input, sessionId, selectedOptions) {
        ...
        for (let depth = 1; depth <= Math.min(selectedOptions.length + 1, this.maxDepth); depth++) {
            const branches = await this.generateLevelInParallel(currentQuestion, { input, history }, depth, path.length > 0 ? path[path.length - 1].id : undefined);
    ...
    async analyzePathInsights(input, path) {
...
//# sourceMappingURL=whys-tree-generator.js.map