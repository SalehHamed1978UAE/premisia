...
export class EPMConverter {
    ...
    async convertToEPM(analysis, decisions, selectedDecisions) {
        ...
        const workstreams = strategyOntologyService.allocateWorkstreams(selectedApproach, selectedMarket);
        const costEstimate = strategyOntologyService.calculateCostEstimate(selectedApproach, selectedMarket);
        const programTitle = await this.generateProgramTitle(analysis, selectedApproach);
        const programDescription = await this.generateProgramDescription(analysis, selectedApproach, selectedMarket);
        const enrichedWorkstreams = await this.enrichWorkstreamsWithTasks(workstreams, analysis, selectedApproach, costEstimate);
...
Return ONLY the description, nothing else.`,
    ...
    async enrichWorkstreamsWithTasks(workstreams, analysis, approach, costEstimate) {
  ...
  "tasks": [
    ...
    extractSuccessCriteria(analysis) {
        if (analysis.porters_five_forces?.key_strategic_priorities) {
            return analysis.porters_five_forces.key_strategic_priorities.map((priority) => `Achieve measurable progress in: ${priority}`);
        }
        return [
...

{
  "risks": [
    ...
    async generateResourceRequirements(workstreams, costEstimate) {
        const resources = [
            {
...
//# sourceMappingURL=epm-converter.js.map