...
export class PESTLEAnalyzer {
    ...
    async extractRisks(frameworkResults) {
        ...
        const negativeFactors = factors.filter(f => f.factor.risks.length > 2 || f.factor.trends.some(t => t.strength > 7));
    ...
    async extractBenefits(frameworkResults) {
        ...
        factors.forEach(({ name, factor, category }) => {
            const strongPositiveTrends = factor.trends.filter(t => t.strength > 6 && !t.description.toLowerCase().includes('threat'));
    ...
    inferRiskTolerance(frameworkResults) {
        ...
        if (totalOpportunities > 5 && totalRisks < 5)
            return 'Aggressive';
        return 'Moderate';
    }
}
//# sourceMappingURL=pestle-analyzer.js.map