...
export class ContextBuilder {
    ...
    static inferBusinessType(insights) {
        ...
        if (contextText.match(/\b(coffee|cafe|shop|store|restaurant|bakery)\b/))
            return 'retail_food_service';
        if (contextText.match(/\b(saas|software|platform|app|tech)\b/))
            return 'saas_platform';
        if (contextText.match(/\b(consulting|service|agency)\b/))
            return 'professional_services';
        if (contextText.match(/\b(manufacturing|factory|production)\b/))
            return 'manufacturing';
        if (contextText.match(/\b(ecommerce|online|marketplace)\b/))
            return 'ecommerce';
...
export class EPMSynthesizer {
    ...
    async synthesize(insights, userContext, namingContext, options) {
        ...
        const intelligentPlanningEnabled = options?.forceIntelligentPlanning === true ||
     ...
     */
    async buildWithOldSystem(insights, userContext, namingContext, initiativeType // EXPLICIT: Initiative type from database
    ) {
     ...
     */
    async buildWithIntelligentPlanning(insights, userContext, namingContext, onProgress, initiativeType // EXPLICIT: Initiative type from database
    ) {
        ...
        const minimalProgram = {
            ...
            resourcePlan,
            financialPlan,
            benefitsRealization: {}, // Will generate after timeline
            riskRegister,
            stageGates: {}, // Will generate after timeline
            kpis: {}, // Will generate after timeline
            stakeholderMap,
            governance,
            qaPlan,
            procurement,
        ...
        try {
            ...
            const planningContext = await ContextBuilder.fromJourneyInsights(insights, insights.frameworkType || 'strategy_workspace', userContext?.sessionId // Pass sessionId if available from userContext
            );
            ...
            console.log('─'.repeat(80));
            const planningResult = await replaceTimelineGeneration(minimalProgram, planningContext, {
    ...
    // ============================================================================
    async generateExecutiveSummary(insights, programName) {
     ...
     */
    async generateWorkstreams(insights, userContext, onProgress, startTime // Optional start time for elapsed tracking
    ) {
        console.log('\n' + '='.repeat(80));
        ...
        try {
            ...
            const workstreams = wbs.workstreams.map((ws, index) => {
            ...
            const workstreams = workstreamInsights.map((insight, index) => {
     ...
     */
    validateEPMData(workstreams, timeline, stageGates) {
        const errors = [];
        const corrections = [];
        ...
        for (const workstream of workstreams) {
            for (const deliverable of workstream.deliverables) {
                if (deliverable.dueMonth < workstream.startMonth || deliverable.dueMonth > workstream.endMonth) {
                    ...
                    deliverable.dueMonth = Math.max(workstream.startMonth, Math.min(deliverable.dueMonth, workstream.endMonth));
                    errors.push(`Deliverable "${deliverable.name}" at M${originalDueMonth} outside workstream ` +
        ...
        const workstreamsWithAdjustedDates = [];
        for (const workstream of workstreams) {
            ...
            for (const depId of workstream.dependencies) {
                ...
                if (dependency.endMonth >= workstream.startMonth) {
                    errors.push(`Invalid dependency: "${workstream.name}" (M${workstream.startMonth}) ` +
                    ...
                    corrections.push(`Adjusted "${workstream.name}" to M${workstream.startMonth}-M${workstream.endMonth} ` +
                ...
                if (workstreamsWithAdjustedDates.includes(workstream.id)) {
                    for (const deliverable of workstream.deliverables) {
                        if (deliverable.dueMonth < workstream.startMonth || deliverable.dueMonth > workstream.endMonth) {
                            const originalDueMonth = deliverable.dueMonth;
                            deliverable.dueMonth = Math.max(workstream.startMonth, Math.min(deliverable.dueMonth, workstream.endMonth));
                            corrections.push(`Re-clamped deliverable "${deliverable.name}" to M${deliverable.dueMonth} ` +
        ...
        for (let i = 1; i < sortedPhases.length; i++) {
            ...
            if (currPhase.startMonth <= prevPhase.endMonth) {
                errors.push(`Phase ${currPhase.phase} "${currPhase.name}" overlaps with ` +
                    `Phase ${prevPhase.phase} "${prevPhase.name}"`);
                const oldStart = currPhase.startMonth;
                currPhase.startMonth = prevPhase.endMonth + 1;
                corrections.push(`Adjusted Phase ${currPhase.phase} start from M${oldStart} to M${currPhase.startMonth}`);
            }
        }
        // 4. Validate stage gates align with phases
        for (const gate of stageGates.gates) {
            const phase = timeline.phases.find(p => gate.month >= p.startMonth && gate.month <= p.endMonth);
            if (!phase) {
                errors.push(`Stage gate ${gate.gate} at M${gate.month} not within any phase`);
                // Auto-correct: Move to nearest phase end
                const nearestPhase = timeline.phases.reduce((prev, curr) => Math.abs(curr.endMonth - gate.month) < Math.abs(prev.endMonth - gate.month) ? curr : prev);
                const oldMonth = gate.month;
                gate.month = nearestPhase.endMonth;
                corrections.push(`Moved gate ${gate.gate} from M${oldMonth} to M${gate.month} (end of Phase ${nearestPhase.phase})`);
            }
        }
     ...
     */
    analyzePlanningGrid(workstreams, timeline) {
        const grid = [];
        const conflicts = [];
        ...
        for (const month of grid) {
            if (month.utilization > 3) {
                conflicts.push(`Month ${month.month} (${month.phase || 'No phase'}): ` +
            ...
            if (month.deliverables.length > 5) {
                conflicts.push(`Month ${month.month}: ${month.deliverables.length} deliverables due ` +
    ...
    async generateTimeline(insights, workstreams, userContext) {
        ...
        if (userContext?.hardDeadlines && userContext.hardDeadlines.length > 0) {
            const earliestDeadline = Math.min(...userContext.hardDeadlines.map(d => Math.ceil((d.date.getTime() - Date.now()) / (30 * 24 * 60 * 60 * 1000))));
        ...
        if (deadlineMonths < totalMonths && userContext?.hardDeadlines) {
            console.warn(`[EPM Synthesis] Hard deadline at M${deadlineMonths} exceeded by corrected schedule (M${totalMonths}). ` +
    ...
    generatePhases(totalMonths, workstreams) {
    ...
    identifyCriticalPath(workstreams) {
    ...
    async generateResourcePlan(insights, workstreams, userContext, initiativeType // EXPLICIT: Initiative type passed from parent
    ) {
        ...
        const internalTeam = await this.generateInternalTeam(estimatedFTEs, workstreams, resourceInsights, finalInitiativeType, insights);
     ...
     */
    async generateInternalTeam(estimatedFTEs, workstreams, resourceInsights, initiativeType, insights) {
        console.log(`[Resource Generation] 🎯 Generating team for initiative type: ${initiativeType}`);
        try {
            // Try LLM-based generation first
            const llmRoles = await this.generateRolesWithLLM(initiativeType, estimatedFTEs, workstreams, insights);
     ...
     */
    async generateRolesWithLLM(initiativeType, estimatedFTEs, workstreams, insights) {
...
Return ONLY valid JSON array of role objects. NO markdown, NO code blocks, ONLY the JSON array.`;
     ...
     */
    getFallbackRoles(initiativeType, estimatedFTEs, workstreams) {
        const timeline = workstreams[0]?.endMonth || 12;
    ...
    async generateFinancialPlan(insights, resourcePlan, userContext) {
    ...
    async generateBenefitsRealization(insights, timeline) {
        const benefitInsights = insights.insights.filter(i => i.type === 'benefit');
        const benefits = benefitInsights.map((insight, idx) => ({
            id: `B${String(idx + 1).padStart(3, '0')}`,
            category: this.categorizeBenefit(insight.content),
            description: insight.content,
    ...
    async generateRiskRegister(insights) {
        const riskInsights = insights.insights.filter(i => i.type === 'risk');
    ...
    async generateStageGates(timeline, riskRegister) {
    ...
    async generateKPIs(insights, benefitsRealization) {
    ...
    async generateStakeholderMap(insights) {
        ...
        if (stakeholders.length < 3) {
            stakeholders.push({ name: 'Executive Sponsor', group: 'Leadership', power: 'High', interest: 'High', engagement: 'Manage closely', communicationPlan: 'Weekly updates' }, { name: 'Program Team', group: 'Execution', power: 'Medium', interest: 'High', engagement: 'Keep informed', communicationPlan: 'Daily standups' }, { name: 'End Users', group: 'Customers', power: 'Medium', interest: 'High', engagement: 'Keep informed', communicationPlan: 'Monthly updates' });
        }
    ...
    async generateGovernance(insights, stakeholderMap) {
        return {
            bodies: [
                {
    ...
    async generateProcurement(insights, financialPlan) {
    ...
    async generateExitStrategy(insights, riskRegister) {
        return {
    ...
    generateDefaultWorkstreams(count) {
        const defaults = [
    ...
    estimateEffort(deliverable) {
        ...
        if (words < 5)
            return '5-10 person-days';
        if (words < 10)
            return '10-20 person-days';
        return '20-40 person-days';
    }
    categorizeBenefit(content) {
        const lower = content.toLowerCase();
        if (lower.includes('revenue') || lower.includes('cost') || lower.includes('$'))
            return 'Financial';
        if (lower.includes('risk') || lower.includes('mitigate'))
            return 'Risk Mitigation';
        if (lower.includes('efficiency') || lower.includes('process'))
            return 'Operational';
        return 'Strategic';
    }
    estimateBenefitValue(insight) {
    ...
    generateMeasurement(content) {
        ...
        if (lower.includes('revenue'))
            return 'Revenue tracking (monthly)';
        if (lower.includes('cost'))
            return 'Cost analysis (quarterly)';
        if (lower.includes('customer'))
            return 'Customer surveys (quarterly)';
        if (lower.includes('market'))
            return 'Market analysis (semi-annual)';
        return 'Performance metrics (quarterly)';
    }
    categorizeRisk(insight) {
        const lower = insight.content.toLowerCase();
        if (lower.includes('technology') || lower.includes('technical'))
            return 'Technical';
        if (lower.includes('market') || lower.includes('competitive'))
            return 'Market';
    ...
    estimateRiskProbability(insight) {
    ...
    estimateRiskImpact(insight) {
        const lower = insight.content.toLowerCase();
        if (lower.includes('critical') || lower.includes('catastrophic'))
            return 'Critical';
    ...
    categorizeStakeholder(content) {
        ...
        if (lower.includes('executive') || lower.includes('leadership'))
            return 'Leadership';
        if (lower.includes('team') || lower.includes('employee'))
            return 'Execution';
        if (lower.includes('partner') || lower.includes('supplier'))
            return 'Partners';
        return 'Other';
    }
    ...
    generateCommunicationPlan(insight) {
        ...
        if (strategy === 'Manage closely')
            return 'Weekly updates, monthly reviews';
        if (strategy === 'Keep satisfied')
            return 'Monthly updates';
        if (strategy === 'Keep informed')
            return 'Quarterly updates, newsletters';
        return 'As needed';
    }
...
`.trim();
     ...
     */
    async generateProgramName(insights, userContext, namingContext) {
        try {
            // Import AI clients
            ...
            if (recommendations.length > 0) {
                const recs = recommendations.slice(0, 2).map((r) => typeof r === 'object' ? r.action : r);
                contextSummary += `\n\nTop Recommendations:\n${recs.join('\n')}`;
            }
...
Generate ONLY the program name, nothing else.`;
            ...
            // Fallback naming
            return this.generateFallbackProgramName(namingContext?.selectedDecisions, namingContext?.decisionsData, namingContext?.framework || 'bmc');
        }
    }
    /**
     ...
     */
    async generateAssignments(epmProgram, epmProgramId) {
        const { synthesizeAssignments } = await import('../services/assignment-engine');
        console.log('[EPM Synthesis] 📋 Generating task assignments...');
        // CRITICAL FIX: Detect if workstreams array contains flat tasks (intelligent planning)
        // vs. nested task structure (legacy system)
        const hasFlatStructure = epmProgram.workstreams.length > 0 &&
            epmProgram.workstreams[0].id?.startsWith('T') &&
            !epmProgram.workstreams[0].tasks;
        let workstreams;
        if (hasFlatStructure) {
            // Flat structure from intelligent planning: each item IS a task
            console.log('[EPM Synthesis] 📋 Detected flat task structure from intelligent planning');
            console.log(`[EPM Synthesis] 📋 Converting ${epmProgram.workstreams.length} tasks into workstream format`);
            workstreams = [{
                    id: 'WS-ALL',
                    name: 'All Program Tasks',
                    description: 'Tasks generated by intelligent planning system',
                    tasks: epmProgram.workstreams.map((task) => ({
                        id: task.id,
                        name: task.name,
                        ...
                        requiredSkills: (() => {
                            // Intelligent planning stores skills in task directly, not in requirements
                            // Check task-level skills first, then requirements array
                        ...
                        estimatedEffort: task.effort || task.duration || 1,
                        priority: task.priority || 'medium',
                    }))
                }];
        }
        else {
            // Normal nested structure: workstreams contain tasks
            console.log('[EPM Synthesis] 📋 Detected nested workstream structure (legacy format)');
            workstreams = epmProgram.workstreams.map((ws) => ({
                id: ws.id,
                name: ws.name,
                description: ws.description || '',
        ...
        const internalResources = (epmProgram.resourcePlan.internalTeam || []).map((member, index) => ({
            id: member.id || `internal-${index}`,
            name: member.role || member.name || 'Team Member',
            role: member.role || 'Team Member',
            type: 'internal_team', // CRITICAL: Must match database enum assignment_resource_type
            skills: member.skills || [],
            availability: member.availability || 100,
            costPerDay: member.costPerUnit || 0,
        }));
        const externalResources = (epmProgram.resourcePlan.externalResources || []).map((ext, index) => ({
            id: ext.id || `external-${index}`,
            name: ext.type || ext.description || 'External Resource',
            role: ext.type || 'External Resource',
            type: 'external_resource', // CRITICAL: Must match database enum assignment_resource_type
            skills: [],
            availability: 100,
        ...
        const assignments = synthesizeAssignments(epmProgramId, workstreams, resourcePlan, {
...
//# sourceMappingURL=epm-synthesizer.js.map