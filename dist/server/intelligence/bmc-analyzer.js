...
export class BMCAnalyzer {
    ...
    async extractCosts(frameworkResults) {
        ...
        if (frameworkResults.keyResources) {
            const resources = this.parseMultiLineContent(frameworkResults.keyResources);
            const humanResources = resources.filter(r => r.toLowerCase().includes('team') ||
    ...
    categorizeResource(resource) {
        ...
        if (lower.includes('engineer') || lower.includes('developer') || lower.includes('technical'))
            return 'Technical';
        if (lower.includes('infrastructure') || lower.includes('platform') || lower.includes('server'))
            return 'Infrastructure';
        if (lower.includes('team') || lower.includes('staff') || lower.includes('talent'))
            return 'Human';
        if (lower.includes('data') || lower.includes('ip') || lower.includes('intellectual'))
            return 'Intellectual Property';
        return 'Other';
    }
    assessContradictionSeverity(contradiction) {
        const lower = contradiction.toLowerCase();
        if (lower.includes('critical') || lower.includes('catastrophic') || lower.includes('fatal'))
            return 'Critical';
    ...
    estimateCostAmount(cost) {
        ...
        if (lower.includes('infrastructure') || lower.includes('platform'))
            return 300000;
        if (lower.includes('personnel') || lower.includes('team') || lower.includes('staff'))
            return 500000;
        if (lower.includes('marketing') || lower.includes('sales'))
            return 200000;
        if (lower.includes('technology') || lower.includes('software'))
            return 150000;
        return 100000; // Default estimate
    }
    inferUrgency(frameworkResults) {
        const allText = Object.values(frameworkResults).join(' ').toLowerCase();
        if (allText.includes('urgent') || allText.includes('immediate'))
            return 'ASAP';
        if (allText.includes('exploration') || allText.includes('experiment'))
            return 'Exploratory';
        return 'Strategic';
    }
    inferBudgetRange(frameworkResults) {
        ...
        if (costMatch || revenueMatch) {
            const amount = Math.max(costMatch ? parseInt(costMatch[1].replace(/,/g, '')) : 0, revenueMatch ? parseInt(revenueMatch[1].replace(/,/g, '')) : 0);
            if (amount < 500000)
                return '$250k - $500k';
            if (amount < 1000000)
                return '$500k - $1M';
...
//# sourceMappingURL=bmc-analyzer.js.map