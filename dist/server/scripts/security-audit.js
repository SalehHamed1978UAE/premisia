...
function detectEncryptionFormat(value) {
...
async function auditTable(pool, tableConfig) {
    ...
    const sampleResult = await pool.query(`SELECT ${fieldList} FROM ${tableName} ORDER BY RANDOM() LIMIT $1`, [sampleSize]);
    const fieldStats = {};
...
function printFieldStatus(fieldName, stats, sampledRecords) {
...
function getTableStatus(result) {
