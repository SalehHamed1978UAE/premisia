...
class AuthorityRegistryService {
     ...
     */
    async recordCorroboration(authorityId) {
        await db.update(authoritySources)
            .set({
    ...
    async getStatistics() {
        ...
        const stats = await db.select({
            totalAuthorities: sql `count(*)`,
            totalHits: sql `sum(${authoritySources.hits})`,
            totalCorroborations: sql `sum(${authoritySources.corroborations})`,
     ...
     */
    async matchDocumentToAuthority(publisherName, countryISO, industry) {
        await this.ensureSeeded();
     ...
     */
    async getFilteredAuthorities(industryFilter, countryFilter) {
        await this.ensureSeeded();
        const allAuthorities = await this.getAllAuthorities();
        // Filter by industry if provided
        let filtered = allAuthorities;
        if (industryFilter) {
            filtered = filtered.filter(auth => auth.industries.includes('all') ||
                auth.industries.some(ind => ind.toLowerCase() === industryFilter.toLowerCase()));
        }
        // Filter by country if provided
        if (countryFilter) {
            filtered = filtered.filter(auth => auth.countries.includes(countryFilter.toUpperCase()));
        }
        return filtered;
    }
    /**
     ...
     */
    async batchMatchDocuments(documents) {
        const matches = [];
        for (const doc of documents) {
            const match = await this.matchDocumentToAuthority(doc.publisher, doc.countryISO, doc.industry);
            matches.push(match);
        }
        return matches;
    }
    ...
    async getSourceGuidance(domain) {
        ...
        const authorities = await this.getFilteredAuthorities(domain.industry, domain.geography);
     ...
     */
    async scoreSource(document, authorities) {
        await this.ensureSeeded();
...
//# sourceMappingURL=authority-registry-service.js.map