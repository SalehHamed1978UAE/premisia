...
export async function registerRoutes(app) {
    ...
    app.get('/api/strategies', isAuthenticated, async (req, res) => {
        try {
            ...
            const strategies = await db
                ...
                journeyCount: sql `COUNT(DISTINCT ${journeySessions.id})`,
                latestJourneyStatus: sql `MAX(${journeySessions.status})`,
                latestJourneyUpdated: sql `MAX(${journeySessions.updatedAt})`,
            })
    ...
    app.get('/api/strategies/latest-understanding', isAuthenticated, async (req, res) => {
        try {
            const userId = req.user.claims.sub;
    ...
    app.get('/api/strategies/:id', isAuthenticated, async (req, res) => {
        try {
            ...
            const [ownershipCheck] = await db
                ...
                .where(and(eq(journeySessions.understandingId, strategyId), eq(journeySessions.userId, userId)))
                .limit(1);
            if (!ownershipCheck) {
            ...
            const programs = sessionIdList.length > 0
                ? await db
                    ...
                    .where(and(inArray(strategyVersions.sessionId, sessionIdList), eq(epmPrograms.userId, userId)))
                : [];
            // Get count of references (with ownership verification)
            const [refCount] = await db
                .select({ count: sql `COUNT(*)` })
                .from(references)
                .where(and(eq(references.understandingId, strategyId), eq(references.userId, userId)));
            res.json({
    ...
    app.get('/api/strategies/:id/references', isAuthenticated, async (req, res) => {
        try {
            const userId = req.user.claims.sub;
            const strategyId = req.params.id;
            ...
            const [understandingData] = await db
                .select()
                .from(strategicUnderstanding)
                .leftJoin(journeySessions, eq(strategicUnderstanding.id, journeySessions.understandingId))
                .where(and(eq(strategicUnderstanding.id, strategyId), eq(journeySessions.userId, userId)));
            ...
            const strategyReferences = await db
                .select()
                .from(references)
                .where(and(eq(references.userId, userId), or(eq(references.understandingId, strategyId), sql `${references.sessionId} IN (
    ...
    app.post('/api/test/intelligent-planning', async (req, res) => {
        try {
            console.log('\n========================================');
            ...
            const epmProgram = await epmSynthesizer.synthesize(insights, userDecisions, namingContext, { forceIntelligentPlanning: true } // Request-scoped override
            );
            console.log('\n========================================');
    ...
    app.post('/api/dev/quick-epm-test', async (req, res) => {
...
Marketing and events: $3k/month`,
            ...
            const epmProgram = await epmSynthesizer.synthesize(insights, userContext, namingContext, { forceIntelligentPlanning: true });
            console.log('\n✅ DEV TEST COMPLETE!\n');
    ...
    app.get("/api/ontology/relationships", requireAuth, async (req, res) => {
        try {
            ...
            if (entity) {
                const relationships = await ontologyService.getEntityRelationships(entity, direction || "both");
    ...
    app.get("/api/session-context", requireAuth, async (req, res) => {
        try {
            ...
            const [activeJourney] = await db.select()
                .from(journeySessions)
                .where(and(eq(journeySessions.userId, userId), eq(journeySessions.status, 'in_progress')))
    ...
    app.post("/api/web-search", async (req, res) => {
        try {
            ...
            const transformedData = {
                organic: (data.web?.results || []).map((result, index) => ({
                    title: result.title || '',
                    link: result.url || '',
    ...
    function extractPublicationDate(html) {
        ...
        function walkJsonLd(obj) {
            ...
            if (typeCheck) {
                ...
                const hasRelevantType = types.some((t) => t && (t.includes('Article') ||
    ...
    app.get('/api/background-jobs/:id', isAuthenticated, async (req, res) => {
        try {
    ...
    app.get('/api/background-jobs/running', isAuthenticated, async (req, res) => {
        try {
            // Validate auth claims exist
            if (!req.user?.claims?.sub) {
                return res.status(401).json({ error: 'Unauthorized' });
    ...
    app.get('/api/background-jobs/by-session/:sessionId', isAuthenticated, async (req, res) => {
        try {
            // Validate auth claims exist
            if (!req.user?.claims?.sub) {
                return res.status(401).json({ error: 'Unauthorized' });
    ...
    app.get('/api/background-jobs/recent', isAuthenticated, async (req, res) => {
        try {
            // Validate auth claims exist
            if (!req.user?.claims?.sub) {
                return res.status(401).json({ error: 'Unauthorized' });
    ...
    app.get('/api/background-jobs/recent-completions', isAuthenticated, async (req, res) => {
        try {
            // Validate auth claims exist
            if (!req.user?.claims?.sub) {
                return res.status(401).json({ error: 'Unauthorized' });
            ...
            const jobs = await db
                .select()
                .from(backgroundJobs)
                .where(and(eq(backgroundJobs.userId, userId), or(eq(backgroundJobs.status, 'completed'), eq(backgroundJobs.status, 'failed')), or(and(eq(backgroundJobs.status, 'completed'), sql `${backgroundJobs.completedAt} >= ${fiveMinutesAgo}`), and(eq(backgroundJobs.status, 'failed'), sql `${backgroundJobs.failedAt} >= ${fiveMinutesAgo}`))))
    ...
    app.get('/api/background-jobs', isAuthenticated, async (req, res) => {
        try {
            // Validate auth claims exist
            if (!req.user?.claims?.sub) {
                return res.status(401).json({ error: 'Unauthorized' });
    ...
    app.delete('/api/background-jobs/:id', isAuthenticated, async (req, res) => {
        try {
            // Validate auth claims exist
            if (!req.user?.claims?.sub) {
                return res.status(401).json({ error: 'Unauthorized' });
    ...
    app.post('/api/background-jobs/cleanup', isAuthenticated, async (req, res) => {
        try {
            // Validate auth claims exist
            if (!req.user?.claims?.sub) {
                return res.status(401).json({ error: 'Unauthorized' });
    ...
    app.post('/api/admin/golden-records', requireAuth, requireRole(['Admin']), async (req, res) => {
        try {
    ...
    app.post('/api/admin/encrypt-legacy-data', requireAuth, requireRole(['Admin']), async (req, res) => {
        try {
