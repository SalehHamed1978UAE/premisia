import { db } from '../db';
import { strategyVersions, strategicUnderstanding, epmPrograms } from '@shared/schema';
 ...
 */
export async function fetchJourneySessionData(sessionId) {
    ...
    if (session.journeyType === 'business_model_innovation') {
        ...
        const fiveWhysRootCauses = session.accumulatedContext?.insights?.rootCauses;
 ...
 */
export async function fetchStrategyVersionData(strategyVersionId) {
 ...
 */
export async function sanitizeGoldenRecordData(data) {
    const sanitizedSteps = await Promise.all(data.steps.map(step => sanitizeGoldenRecordStep(step)));
    return {
        ...data,
 ...
 */
export async function saveGoldenRecordToFile(data, notes) {
 ...
 */
export function generateDiffSummary(baseline, current) {
 ...
 */
export function prepareGoldenRecordForAPI(data, notes, promoteAsCurrent = false, parentVersion) {
    return {
 ...
 */
export async function pushGoldenRecordToKnowledgeGraph(data) {
    try {
...
//# sourceMappingURL=golden-records-service.js.map