...
export function encryptWithDataKey(plaintext, dataKey) {
    try {
...
export async function encryptKMS(text) {
    if (!text)
        return null;
...
export async function decryptKMS(encryptedData) {
    if (!encryptedData)
        return null;
...
export async function encryptJSONKMS(obj) {
    if (!obj)
        return null;
    return encryptKMS(JSON.stringify(obj));
}
export async function decryptJSONKMS(encryptedData) {
    if (!encryptedData)
        return null;
...
export async function validateKMSSetup() {
    try {
        const client = getKMSClient();
        const keyId = getKMSKeyId();
...
//# sourceMappingURL=kms-encryption.js.map