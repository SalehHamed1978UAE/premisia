...
export const strategicEntities = pgTable("strategic_entities", {
    ...
    embeddingIdx: index("idx_strategic_entities_embedding").using('ivfflat', table.embedding.op('vector_cosine_ops')).with({ lists: 100 }),
    textSearchIdx: index("idx_strategic_entities_text_search").using('gin', sql `to_tsvector('english', ${table.claim} || ' ' || COALESCE(${table.source}, '') || ' ' || COALESCE(${table.evidence}, ''))`),
}));
