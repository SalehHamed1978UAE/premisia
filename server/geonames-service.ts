...
import type { Geography } from '../shared/schema.js';
...
interface GeoNamesTelemetry {
  geoNamesCalls: number;
  geoNamesFailures: number;
  llmFallbacks: number;
...
class GeoNamesService {
  ...
  private telemetry: GeoNamesTelemetry = {
    geoNamesCalls: 0,
    geoNamesFailures: 0,
    llmFallbacks: 0,
    circuitOpenCount: 0,
   ...
   */
  private recordGeoNamesFailure(): void {
   ...
   */
  private determineScope(result: Partial<GeocodingResult>): 'local' | 'regional' | 'national' | 'international' {
    if (result.city && result.cityGeoNameId) return 'local';
    if (result.region && result.regionISO) return 'regional';
    if (result.country && result.countryISO2) return 'national';
   ...
   */
  private async geocodeWithGeoNames(locationText: string): Promise<Partial<GeocodingResult>> {
    ...
    const searchResponse = await fetch(
      `http://api.geonames.org/searchJSON?q=${encodeURIComponent(locationText)}&maxRows=1&username=${username}`
    );
    ...
    const result: Partial<GeocodingResult> = {
   ...
   */
  private async geocodeWithLLM(locationText: string): Promise<Partial<GeocodingResult>> {
...
If you cannot determine the location, use null for unknown fields.`;
   ...
   */
  async geocode(locationText: string): Promise<Geography> {
    let result: Partial<GeocodingResult> = {};
    let provider: 'geonames' | 'llm' = 'geonames';
   ...
   */
  async geocodeBatch(locations: string[]): Promise<Geography[]> {
    const results: Geography[] = [];
   ...
   */
  getTelemetry(): GeoNamesTelemetry {
