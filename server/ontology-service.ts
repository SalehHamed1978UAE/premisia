...
export class OntologyService {
  async getEntityDefinition(entityName: EPMEntity) {
  ...
  async getEntityRelationships(entityName: EPMEntity, direction: "from" | "to" | "both" = "both") {
    if (direction === "from") {
      return await db
        .select()
      ...
      return await db
        ...
        .where(
          or(
            eq(ontologyRelationships.fromEntity, entityName),
            eq(ontologyRelationships.toEntity, entityName)
          )
        );
    }
  }
  ...
  async getCascadeImpacts(trigger?: string) {
    if (trigger) {
      return await db
        .select()
  ...
  async getDomainTerm(term: string) {
  ...
  async validateEntityData(entityName: EPMEntity, data: any) {
    ...
    const results = {
      isValid: true,
      errors: [] as Array<{ rule: string; message: string; autoFix?: string }>,
      warnings: [] as Array<{ rule: string; message: string }>,
    };
  ...
  private evaluateValidationRule(ruleId: string, ruleData: any, data: any): boolean {
  ...
  async checkCompleteness(entityName: EPMEntity, data: any) {
    ...
    const results = {
      ...
      missingFields: [] as Array<{ field: string; importance: string; description: string }>,
    };
  ...
  async getFullEntityContext(entityName: EPMEntity) {
