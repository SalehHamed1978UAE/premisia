...
import type { PESTLEFactors, PESTLEClaim } from './pestle-claims-service.js';
import type { AssumptionComparison } from './assumption-comparison-service.js';
...
Focus on actionable insights that can inform strategic planning and decision-making.`;
   ...
   */
  private parseSynthesisResponse(text: string): TrendSynthesis {
    try {
   ...
   */
  createTelemetry(): TrendTelemetry {
    return {
      totalLatencyMs: 0,
      llmCalls: 0,
      totalTokens: 0,
   ...
   */
  trackLLMCall(telemetry: TrendTelemetry, provider: string, tokens?: number) {
    telemetry.llmCalls++;
    if (tokens) {
    ...
    if (validProviders.includes(provider as any)) {
      telemetry.providerUsage[provider as 'openai' | 'anthropic' | 'gemini']++;
    } else {
   ...
   */
  trackAPICall(telemetry: TrendTelemetry) {
    telemetry.apiCalls++;
  }
   ...
   */
  trackCacheHit(telemetry: TrendTelemetry) {
    telemetry.cacheHits++;
  }
   ...
   */
  trackRetry(telemetry: TrendTelemetry) {
    telemetry.retries++;
  }
   ...
   */
  addLatency(telemetry: TrendTelemetry, latencyMs: number) {
