...
import type { InsertTaskAssignment, TaskAssignment } from '@shared/schema';
...
export interface AssignmentSynthesisOptions {
  preferredAllocation?: number; // Default allocation % (default: 100)
  skillMatchRequired?: boolean; // Require skill matching (default: true)
  autoBalance?: boolean; // Automatically balance workload (default: false)
  confidence?: 'high' | 'medium' | 'low'; // Default confidence level
}
 ...
 */
export function synthesizeAssignments(
  epmProgramId: string,
  workstreams: Workstream[],
  resourcePlan: ResourcePlan,
  options: AssignmentSynthesisOptions = {}
): Omit<InsertTaskAssignment, 'id' | 'createdAt' | 'updatedAt'>[] {
  const {
  ...
  const assignments: Omit<InsertTaskAssignment, 'id' | 'createdAt' | 'updatedAt'>[] = [];
  ...
  for (const workstream of workstreams) {
    for (const task of workstream.tasks) {
      ...
      const matchedResource = findBestMatchingResource(
        task,
        resources,
        skillMatchRequired
      );
...
): (Resource & { matchScore: number; matchedSkills: string[] }) | null {
  let bestMatch: (Resource & { matchScore: number; matchedSkills: string[] }) | null = null;
  let bestScore = -1;
  ...
  for (const resource of resources) {
    ...
    if (task.requiredSkills && task.requiredSkills.length > 0) {
      ...
      const matchCount = task.requiredSkills.filter(skill => {
        const hasSkill = resourceSkills.some(rs => 
...
export function validateAssignment(
  assignment: Omit<InsertTaskAssignment, 'id' | 'createdAt' | 'updatedAt'>,
  ...
  const resourceAssignments = existingAssignments.filter(
    a => a.resourceId === assignment.resourceId &&
         a.status === 'active' &&
         a.id !== (assignment as any).id // Exclude self if editing
  );
  ...
  const conflictingAssignments = resourceAssignments.filter(a => {
    ...
    return (
      isWithinInterval(existingInterval.start, assignmentInterval) ||
  ...
  if (conflictingAssignments.length > 0) {
    ...
    if (totalAllocation > 100) {
      warnings.push(
        `Resource ${assignment.resourceName} is overallocated (${totalAllocation}%) during this period. ` +
...
export function validateAllAssignments(
  assignments: (Omit<InsertTaskAssignment, 'id' | 'createdAt' | 'updatedAt'> | TaskAssignment)[]
): { valid: boolean; results: Map<string, AssignmentValidationResult> } {
  const results = new Map<string, AssignmentValidationResult>();
  let allValid = true;

  for (let i = 0; i < assignments.length; i++) {
    const assignment = assignments[i];
    const key = (assignment as any).id || `temp-${i}`;
    
    const result = validateAssignment(
      assignment as Omit<InsertTaskAssignment, 'id' | 'createdAt' | 'updatedAt'>,
      assignments.filter(a => a !== assignment) as TaskAssignment[]
    );
...
export function calculateResourceWorkload(
  ...
  const averageAllocation =
    timeline.reduce((sum, t) => sum + t.allocation, 0) / timeline.length;
 ...
 */
export function getAssignmentSummary(assignments: TaskAssignment[]): {
  totalAssignments: number;
 ...
 */
export function findUnassignedTasks(
  workstreams: Workstream[],
  assignments: TaskAssignment[]
): WorkstreamTask[] {
  const assignedTaskIds = new Set(assignments.map(a => a.taskId));
  const unassigned: WorkstreamTask[] = [];
