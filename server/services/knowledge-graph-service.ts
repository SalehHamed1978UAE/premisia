 ...
 */
export async function upsertNodes(nodes: NodeUpsert[]): Promise<void> {
 ...
 */
export async function upsertRelationships(rels: RelUpsert[]): Promise<void> {
 ...
 */
export async function upsertJourneySession(data: JourneySessionNode): Promise<void> {
 ...
 */
export async function upsertFrameworkOutput(data: FrameworkOutputNode): Promise<void> {
  const nodes: NodeUpsert[] = [{
 ...
 */
export async function upsertDecision(data: DecisionNode): Promise<void> {
  const nodes: NodeUpsert[] = [{
    label: 'Decision',
    matchOn: { id: data.id },
    properties: data,
 ...
 */
export async function upsertDecisionOptions(data: DecisionOptionNode[]): Promise<void> {
  if (data.length === 0) return;

  const nodes: NodeUpsert[] = data.map(option => ({
  ...
  const rels: RelUpsert[] = data.map(option => ({
 ...
 */
export async function upsertProgram(data: ProgramNode): Promise<void> {
  const nodes: NodeUpsert[] = [{
    label: 'Program',
    matchOn: { id: data.id },
    properties: data,
 ...
 */
export async function linkJourneyToIncentives(journeyId: string, incentiveIds: string[]): Promise<void> {
  if (incentiveIds.length === 0) return;

  const rels: RelUpsert[] = incentiveIds.map(incentiveId => ({
 ...
 */
export async function linkJourneyToRegulations(journeyId: string, regulationIds: string[]): Promise<void> {
  if (regulationIds.length === 0) return;

  const rels: RelUpsert[] = regulationIds.map(regulationId => ({
    from: { label: 'Program', matchOn: { id: journeyId } },
    type: 'CONSTRAINED_BY',
 ...
 */
export async function createEvidenceLinks(
  decisionId: string,
  evidenceList: Array<{ id: string; referenceId?: string; snippet?: string; confidence?: number; sourceType: string; origin: string }>
): Promise<void> {
  if (evidenceList.length === 0) return;

  // Create evidence nodes
  const evidenceNodes: NodeUpsert[] = evidenceList.map(evidence => ({
    label: 'Evidence',
  ...
  const decisionRels: RelUpsert[] = evidenceList.map(evidence => ({
  ...
  const referenceRels: RelUpsert[] = evidenceList
...
export async function getAvailableIncentives(params: {
  ...
  industryId?: string;
  limit?: number;
}): Promise<AvailableIncentiveResult[]> {
