...
async function testAsanaCase() {
  ...
  try {
    ...
    const allSourcesValid = entities.every(entity => 
      testInput.toLowerCase().includes(entity.source.toLowerCase())
    );
    ...
    if (hallucinationFound) issues.push("Hallucinations detected");
    if (!allSourcesValid) issues.push("Invalid sources");
    if (entities.length === 0) issues.push("No entities extracted");
    if (entities.length > 8) issues.push("Too many entities (should be 3-8)");
