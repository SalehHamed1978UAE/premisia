...
export interface AssessmentResult {
  isRealistic: boolean;
  concerns: Array<{
    category: 'timeline' | 'budget' | 'resources' | 'risk' | 'scope' | 'quality';
...
export class AssessmentService {
  ...
  async assessProgram(program: any, relatedData?: {
    tasks?: any[];
    stageGates?: any[];
    fundingSources?: any[];
    benefits?: any[];
    ...
    const concerns: AssessmentResult['concerns'] = [];
    ...
    if (relatedData?.risks) {
      const highRisks = relatedData.risks.filter((r: any) => r.priority === 'High' || r.priority === 'Critical');
      const openHighRisks = highRisks.filter((r: any) => r.status === 'Open');
    ...
    return {
      ...
      benchmarks: {
        ...
        current: {
          ...
          openHighRisks: relatedData?.risks?.filter((r: any) => 
            (r.priority === 'High' || r.priority === 'Critical') && r.status === 'Open'
          ).length || 0
        },
  ...
  async assessTask(task: any, program?: any): Promise<AssessmentResult> {
    const concerns: AssessmentResult['concerns'] = [];
  ...
  async assessRisk(risk: any, mitigations?: any[]): Promise<AssessmentResult> {
    const concerns: AssessmentResult['concerns'] = [];
    ...
    return {
      ...
      benchmarks: {
        ...
        current: {
          ...
          completedMitigations: mitigations?.filter((m: any) => m.status === 'Completed').length || 0
        },
  ...
  async assessBenefit(benefit: any, program?: any): Promise<AssessmentResult> {
    const concerns: AssessmentResult['concerns'] = [];
  ...
  async assessKpi(kpi: any, measurements?: any[]): Promise<AssessmentResult> {
    const concerns: AssessmentResult['concerns'] = [];
    ...
    if (measurements && measurements.length > 0) {
      const latestMeasurement = measurements.reduce((latest, m) => 
        new Date(m.measurementDate) > new Date(latest.measurementDate) ? m : latest
      , measurements[0]);
  ...
  async assessResource(resource: any): Promise<AssessmentResult> {
    const concerns: AssessmentResult['concerns'] = [];
  ...
  private calculateDaysBetween(start: Date | string, end: Date | string): number {
