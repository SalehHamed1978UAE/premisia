...
// ============================================================================
router.get('/resource/:resourceId/workload', async (req: Request, res: Response) => {
  try {
    ...
    if (programId) {
      ...
      assignments = await db
        ...
        .where(
          and(
            eq(taskAssignments.resourceId, resourceId),
            eq(taskAssignments.epmProgramId, programId as string)
          )
        );
    } else {
...
router.patch('/:id', async (req: Request, res: Response) => {
  try {
    ...
    const otherAssignments = await db
      ...
      .where(
        and(
          eq(taskAssignments.epmProgramId, existing.epmProgramId),
          ne(taskAssignments.id, id) // Exclude self
        )
      );
...
// ============================================================================
router.get('/validate/:programId', async (req: Request, res: Response) => {
  try {
    const { programId } = req.params;
